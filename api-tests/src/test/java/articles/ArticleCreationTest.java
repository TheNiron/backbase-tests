...
public class ArticleCreationTest extends Authentication {
  ...
  public void createNewArticleAndValidateResponse() throws IOException {
    ...
    Validations.ValidateValuesInResponseBody(
        Constants.USERNAME, Constants.AUTHOR_USERNAME_JSONPATH, response);
    Validations.ValidateValuesInResponseBody(
        articleTitle, Constants.ARTICLE_TITLE_JSONPATH, response);
    Validations.ValidateValuesInResponseBody(
        articleAbout, Constants.ARTICLE_DESCRIPTION_JSONPATH, response);
    Validations.ValidateValuesInResponseBody(
        articleContent, Constants.ARTICLE_BODY_JSONPATH, response);
  ...
  public void createNewArticleWithExistingTitle() throws IOException {
    ...
    // Validate response code is 200
    Validations.ValidateResponseCode(response.getStatusCode(), Constants.STATUS_CODE_200);
    String slugOfFirstArticle =
        response.body().jsonPath().getString(Constants.ARTICLE_SLUG_JSONPATH);
  ...
  public void invokeCreateArticleWithInvalidJWTtoken() throws IOException {
    ...
    articleTag =
        commonUtils
            ...
            .replace("\"", "");
    String invalidJWT =
        "eyJhbGciOiJIUzI1NiIsIc2342IkpXVCJ9.eyJpZCI6IjYzMjFiZjNhMjA5MmNmYzYwNjhlNzk4YyIsInVzZXJuYW1lIjoiaGFycnkiLCJleHAiOjE2Njg1MjExNTEsImlhdCI6MTY2MzMzNzE1MX0.zd6pbBUPUZ8qd-1PwvxYuchRb7rHwbOQqXgI4uWJnKY";

    logger.log(Level.INFO, "Invoke create article request");
    response =
        article.createArticleRequest(
            articleTitle, articleAbout, articleContent, articleTag, invalidJWT);
